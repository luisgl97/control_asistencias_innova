...
module.exports = async (usuarioData, usuarioRepository) => {
  const { success, message: error, usuario } = Usuario.crear(usuarioData);
  if (!success)
    return {
      codigo: 400,
      respuesta: {
  ...
  const existeCorreo = await usuarioRepository.obtenerPorEmail(usuario.email);
  if (existeCorreo) {
    return {
      codigo: 400,
      respuesta: {
  ...
  const existeDni = await usuarioRepository.obtenerPorDni(usuario.dni);
  if (existeDni) {
    return {
      codigo: 400,
      respuesta: {
  ...
  const usuarioTransformado = await mapeoUsuarioPasswordEncriptado(usuario);
