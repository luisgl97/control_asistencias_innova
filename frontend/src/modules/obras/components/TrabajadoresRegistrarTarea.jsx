                        ...
                        loading ? (
                            ...
                            <div className="space-y-3 max-h-96 overflow-y-auto p-2">
                                {trabajadoresFiltrados.map((trabajador) => (
                                    !tareas.some(tarea => tarea.trabajadores.some(t => t.id === trabajador.id)) && (
                                                ...
                                                <div className="flex flex-wrap gap-1">
                                                    {tareas.map((tarea, index) => (
                                                        <Button
                                                            ...
                                                            variant="outline"
                                                            size="sm"
                                                            className={`text-xs h-7 px-2 ${tarea.descripcion ? 'border-purple-200 text-purple-600 hover:bg-purple-50 hover:border-purple-300' : 'bg-gray-200 border-gray-200 text-gray-400 cursor-not-allowed'}`}
                                                            disabled={!tarea.descripcion}
                                                        >
